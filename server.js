...
//              tunge sider bag proxyen var Cookiebot ikke klar endnu, så accepten no-op'ede og der
//              blev aldrig skrevet en gyldig TCF-streng. Fix: klik den RIGTIGE "Tillad alle"-knap
//              (som et menneske) + API-accept, og VERIFICÉR at TCF-strengen faktisk er skrevet —
//              gentag op til 4 gange. giveConsent bruges også på live-stien, så auto-accept i live
//              forbedres direkte. Live-rendering urørt.
...
//              så vi skjulte annoncen → kun rødt frame. Fjernet den regel. Verificeret live: så
//              renderer Lotto. (Diagnose-hooks + SwiftShader beholdt indtil du bekræfter; ryddes/
//              testes bagefter.) Kun preview; Live urørt.
...
//              og forvred i stedet layoutet. previewStripInit skjuler nu KUN Adnamis preview-
//              værktøjs-widgets (theme-switcher, QR, view-on-mobile, page-header, havbaggrund,
//              consent) — artikel-indhold og annonce røres ikke. Adnamis eget naturlige mobil-
//              layout (artikel + annonce i rigtig højde + artikel) får lov at stå urørt. Kun
//              preview — Live er urørt. (Bånd-fjernelsen skal ske via Adnamis content, ikke via
//              device- eller vindue-hacks; se næste skridt.)
...
//              er urørt (previewMode=false → nøjagtig 3.0-adfærd).
...
// Cookiebot (IAB TCF) sites: we click the REAL "Allow all" button (exactly what a human does,
// which grants full per-vendor TCF consent) AND fire the API — then VERIFY a valid TCF string
// was actually written, retrying a few times. The old code fired once after a fixed 400 ms wait;
// on heavier sites behind the proxy (e.g. videnskab.dk) Cookiebot wasn't ready yet, so the accept
// no-op'd and ads never filled — while a manual click worked. Verify-and-retry fixes that race.
async function isConsented(page) {
  return await page.evaluate(() => new Promise((resolve) => {
    try {
      if (typeof window.__tcfapi !== "function") return resolve(false);
      let settled = false;
      window.__tcfapi("getTCData", 2, (d, ok) => { settled = true; resolve(!!(ok && d && d.tcString && d.tcString.length > 20)); });
      setTimeout(() => { if (!settled) resolve(false); }, 2000);
...
/* ------------------------------------------------------------------ *
 * "Preview only" (Tilstand) — 3.1                                      *
 * Adnami's RMB engine only renders a creative on an authorised host,   *
 * so we render Adnami's OWN preview page (preview.adnami.io) top-level  *
 * and strip its surrounding chrome so ONLY the creative sits on a       *
 * neutral wireframe page. Fully ISOLATED: only runs when the client     *
 * sends preview:1. Live is never touched.                              *
 * ------------------------------------------------------------------ */
const PREVIEW_HOST = "preview.adnami.io";
// Runs INSIDE the Adnami preview page (via addInitScript, before Adnami's own JS). Hides ONLY
// Adnami's presentation chrome via EXPLICIT selectors — it deliberately does NOT touch large/fixed/
...
function previewStripInit() {
  ...
  var HIDE =
    ...
    // BEMÆRK: vi skjuler IKKE '.mobile-background' længere. For takeover-kreativer (fx Lotto/
    // be455b06, format canvasmobile) er '.mobile-background' selve KREATIVETS container — at skjule
    // den efterlod kun det røde baggrunds-frame. Verificeret live: fjernes reglen, renderer kreativet.
